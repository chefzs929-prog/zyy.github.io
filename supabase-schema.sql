...
--        ① 把本文件全文粘贴进去，点 Run —— 会建好两张表、两个读取函数；
--        ② 再照「第 3 部分」的说明，改成自己的口令、单独跑那一条 INSERT。
...
-- ─────────────────────────────────────────────
-- 第 3 部分：设置你的查看口令 —— 默认已注释掉，必须改完再单独跑
-- ⚠️ 两条铁律：
--   ① 把下面的「换成你的口令」替换成你自己要用的口令；
--   ② 只选中那一条 INSERT 单独运行，不要跟上面的建表语句一起跑。
--   为什么这么强调：这句占位文字是公开写在仓库里的。不改就整段跑，
--   口令就等于「换成你的口令」，任何人都能进后台读走全部反馈。
--   口令只以 md5 存在数据库里，明文不落盘，网页代码里也没有。
--   想改口令随时重跑那一条（带 ON CONFLICT，可重复运行）。
-- ─────────────────────────────────────────────
...
-- 跑完用这句自检，应该返回 true：
-- SELECT k, pass_hash = md5('wbfb::' || '换成你的口令') AS 校验通过 FROM admin_secret;


-- ─────────────────────────────────────────────
-- 附：登录提示「口令不正确」时的自检
--     把中间那段换成你在网页上输入的那串，跑完对一眼就知道问题在哪
-- ─────────────────────────────────────────────
-- SELECT
--   (SELECT count(*) FROM admin_secret)                    AS 口令行数,
--   (SELECT pass_hash FROM admin_secret WHERE k='owner')   AS 库里存的哈希,
--   md5('wbfb::' || '你在网页上输入的口令')                  AS 你输入的哈希;
-- 口令行数 = 0   → 第 3 部分没跑成功，重跑一次
-- 两个哈希不同   → 设置的口令和输入的口令不是同一个（注意全角字符、首尾空格）
